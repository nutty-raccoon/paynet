...
pub async fn run_e2e() -> Result<()> {
    ...
    let node_id =
        wallet::node::register(db_pool.clone(), &mut node_client.client, &node_client.url).await?;
    let mut wallet_ops = WalletOps::new(db_pool.clone(), node_id, node_client.client);
    ...
    assert_eq!(wad_record.node_url, node_client.url.to_string());
    // Recive
    ...
    for record in wad_records {
        ...
        assert_eq!(record.node_url, node_client.url.to_string());
    }
    ...
    let node_id =
        wallet::node::register(db_pool.clone(), &mut node_client.client, &node_client.url).await?;
    let wallet_ops = WalletOps::new(db_pool.clone(), node_id, node_client.client);
