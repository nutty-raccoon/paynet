        ...
        match event.clone() {
            Message::Payment(payment_events) => {                
                for payment_event in payment_events {
                    let mqid = match db_node::mint_quote::get_quote_id_by_invoice_id(db_conn, payment_event.invoice_id.to_string()).await? {
                        None => {
                            continue;
                        }
                        Some(mint_quote_id) => mint_quote_id
                    };
                    db_node::payment_event::insert_new_payment_event(
                        db_conn,
                        &payment_event
                    )
                    .await?;
                    ...
                    .map(|r|-> Result<primitive_types::U256, Error> {
                        let strk_256 = StarknetU256 {
                            low: Felt::from_str(&r.amount_low).map_err(|e|
                                ServiceError::Indexer(e.into())
                            )?,
                            high: Felt::from_str(&r.amount_high).map_err(|e|
                                ServiceError::Indexer(e.into())
                            )?
                        };
                    ...
                    .try_fold(primitive_types::U256::zero(), |acc, x| match x {
